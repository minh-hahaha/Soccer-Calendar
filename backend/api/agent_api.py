...
fantasy_agent = FantasyFootballAgent(DB_CONNECTION)
...
async def ai_analyze_fantasy_strategy(
    ...
    analysis_results = {
        ...
        "ai_powered": True,
        "model_confidence": "high" if min(fantasy_agent.model_performance.values(), key=lambda x: x['r2'])['r2'] > 0.3 else "medium",
        "recommendations": {},
        "insights": {
...
async def get_player_predictions(
    ...
    return {
        ...
        "model_info": {
            "confidence": "high" if min(fantasy_agent.model_performance.values(), key=lambda x: x['r2'])['r2'] > 0.3 else "medium",
...
async def get_ai_transfer_targets(
    ...
    for player in players:
        try:
            pred = fantasy_agent.predict_player_performance(player) # next 5 gameweeks
...
async def get_ai_market_analysis():
    ...
    for player in players:
        if player.get('minutes', 0) < 50: # only consider player > 50 mins playing
            continue
            ...
            if fantasy_agent.models:
                ...
                if predicted_value > current_value * 1.2 and float(player.get('selected_by_percent', 0)) < 10:
                    ...
                    overvalued_picks.append({
                        ...
                        "ownership": float(player.get('selected_by_percent', 0)),
                        "risk_warning": bool(pred.risk_score > 0.5)
                    })
        except:
            continue
...
async def get_ai_fixture_analysis(
    ...
    if not fantasy_agent.bootstrap_data or not fantasy_agent.fixtures_data:
        if not fantasy_agent.initialize_data():
            raise HTTPException(status_code=503, detail="Failed to initialize data")
    
    teams = fantasy_agent.bootstrap_data['teams']
    ...
    for team in teams:
        team_fixtures = fantasy_agent._get_team_fixtures(team['id'], gameweeks_ahead)
        ...
        if team_fixtures:
            ...
            fixture_analysis.append({
                ...
                "fixture_details": [
                    {
                        "gameweek": f['event'],
                        "opponent": fantasy_agent._get_team_name(f['team_a'] if f['team_h'] == team['id'] else f['team_h']),
...
async def ai_agent_health():
    ...
    model_status = "ready" if fantasy_agent.models else "not_trained"
    data_status = "loaded" if fantasy_agent.bootstrap_data else "not_loaded"
    ...
    return {
        ...
        "current_gameweek": fantasy_agent.current_gameweek,
        "models_available": list(fantasy_agent.models.keys()) if fantasy_agent.models else [],
        "capabilities": [
            "player_predictions",
        ...
        "database_connected": fantasy_agent.db_engine is not None
    }
...
def generate_ai_summary(analysis_results: dict, risk_tolerance: str) -> str:
    ...
    summary_parts.append("🚀 **Powered by**: Machine Learning predictions on 5 seasons of data")
